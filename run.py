...
class ngentod:
		...
		try:
			...
			for softek in memek:
				if "&amp;refid=" in softek[0]:
					self.id.append(re.findall("id=(.*?)&",softek[0])[0]+"[sloerdgans]"+softek[1])
				elif "profile.php?" in softek[0]:
					self.id.append(re.findall("id=(.*)",softek[0])[0]+"[sloerdgans]"+softek[1])
				elif "?refid=" in softek[0]:
					self.id.append(re.findall("(.*?)\?refid=",softek[0])[0]+"[sloerdgans]"+softek[1])
				else:
					self.id.append(softek[0]+"[sloerdgans]"+softek[1])
	...
	def babaturan(self,hencet):
		try:
			...
			for softek in memek:
				if "?uid" in softek[0]:
					self.id.append(re.findall("uid\=(.*?)\&",softek[0])[0]+"[sloerdgans]"+softek[1])
				else:
					self.id.append(re.findall("\/(.*?)\?fref",softek[0])[0]+"[sloerdgans]"+softek[1])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
			if "Lihat selengkapnya" in kontol:
						...
						for c in b:
							if "profile.php" in c[0]:
								...
								if d in self.id:
									...
									self.id.append(d+"[sloerdgans]"+c[1])
							else:
		...
		try:
			...
			for softek in memek:
				if "profile.php?" in softek[1]:
					self.id.append(re.findall("id=(.*?)&amp;refid",softek[1])[0]+"[sloerdgans]"+softek[2])
				else:
					self.id.append(re.findall("(.*?)\?refid=",softek[1])[0]+"[sloerdgans]"+softek[2])
				print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if len(self.id)==jum:
					true=True
					break
			...
			if "Semua 0" in kontol:
				...
				for softek in memek:
					if "profile.php?" in softek[0]:
						...
						self.id.append(re.findall("/(.*)",softek[0])[0]+"[sloerdgans]"+softek[1])
					print(f"\r[*] Mengumpulkan {len(self.id)} ID",end="")
				if "Lihat Selengkapnya" in kontol:
				...
				if "profile.php?" in softek[0]:
					...
					if "/" in tol[-1:]:
						...
						if tol in self.id:
							...
							self.id.append(tol+"[sloerdgans]"+softek[1])
				else:
